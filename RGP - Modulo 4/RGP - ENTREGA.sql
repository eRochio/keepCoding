-- Primero de todo, creamos nuestro esquema:

create schema keepcoding_coches;

-- A continuación, creamos una tabla vacia donde se almacenaran los datos de partida:
...
-- Se insertan los datos que se nos ha proporcionado:
...
-- Procedemos a hacer las tablas de las entidades de relacion:

create table keepcoding_coches.grupos_empresariales(
	marca varchar(30) primary key,
	grupo_empresarial varchar(50) not NULL
);

create table keepcoding_coches.aseguradora(
	id serial primary key,
	aseguradora varchar(50) not NULL
);

create table keepcoding_coches.moneda(
	id serial primary key,
	moneda varchar(30) not NULL
);

create table keepcoding_coches.modelo(
	id serial primary key,
	modelo varchar(30) not NULL
);

create table keepcoding_coches.color(
	id serial primary key,
	color varchar(20) not NULL
);


-- AQUI SE COMIENZA CON LOS FOREIGN KEYS ---

create table keepcoding_coches.seguro(
	id serial primary key,
	n_poliza int4,
	fecha_alta_seguro date null,
	...
	constraint aseguradora_fk foreign key (id_aseguradora) references aseguradora(id)
);

create table keepcoding_coches.marca(
	id serial primary key,
	marca varchar(50) not null,
	constraint grupos_empresariales_fk foreign key (marca) references grupos_empresariales(marca)
);


create table keepcoding_coches.revisiones(
	id serial primary key,
	matricula VARCHAR(10),
	f_revision date,
	importe VARCHAR(20),
	...
	constraint moneda_fk foreign key (id_moneda) references moneda(id)
);


create table keepcoding_coches.vehiculo(
	matricula VARCHAR(10) primary key,
	kms_total int not null,
	f_compra date not null,
	
	id_marca int not null,
	constraint marca_fk foreign key (id_marca) references marca(id),
	
	id_modelo int not null,
	constraint modelo_fk foreign key (id_modelo) references modelo(id),
	
	id_color int not null,
	constraint color_fk foreign key (id_color) references color(id),
	
	id_seguro int not null,
	constraint seguro_fk foreign key (id_seguro) references seguro(id),
	
	id_revisiones int not null,
	constraint revisiones_fk foreign key (id_revisiones) references revisiones(id)
);


-- Una vez que tenemos las tablas, comenzamos a insetarle los datos procedentes de la primera tabla 'coches':

insert into keepcoding_coches.modelo (modelo)
select modelo from keepcoding_coches.coches group by modelo;

insert into keepcoding_coches.color  (color)
select color from keepcoding_coches.coches group by color;

insert into keepcoding_coches.grupos_empresariales (marca,grupo_empresarial)
select  coches.marca, coches.grupo  from keepcoding_coches.coches group by marca, grupo   ;

insert into keepcoding_coches.aseguradora  (aseguradora)
select aseguradora  from keepcoding_coches.coches group by aseguradora ;

insert into keepcoding_coches.moneda  (moneda)
select moneda from keepcoding_coches.coches group by moneda;

insert into keepcoding_coches.marca  (marca)
select marca from keepcoding_coches.grupos_empresariales  group by marca;

-----  comenzamos con los JOIN de las tablas:   --------------------------
/* Nos faltan las tablas en las que aparecen las relaciones, 
 * estos datos se relacionan mediante 'JOIN'.
 */


--Tabla de seguro:
/*he visto que hay polizas que se repiten y es del mismo vehiculo, por ello
he utilizado 'select distinct', para evitar duplicados
*/
insert into keepcoding_coches.seguro (n_poliza, fecha_alta_seguro,matricula, id_aseguradora)
select  distinct 
	c.n_poliza, 
	c.fecha_alta_seguro, 
	c.matricula, 
	keepcoding_coches.aseguradora.id  
from keepcoding_coches.coches c 
inner join keepcoding_coches.aseguradora  on c.aseguradora  = keepcoding_coches.aseguradora.aseguradora  
order by c.matricula, fecha_alta_seguro asc 


--Tabla de revisiones:
insert into keepcoding_coches.revisiones (matricula, f_revision, importe, kms_revision, id_moneda)
select 
	c.matricula, 
	c.fecha_revision, 
	c.importe_revision, 
	...
	m.id 
from keepcoding_coches.coches c 
inner join keepcoding_coches.moneda m  on c.moneda = m.moneda  
order by matricula,c.fecha_revision  


/* aqui selecciono el maximo id para una matricula en la tabla de revisiones,
y en la tabla de seguro, asi obtenemos los datos de las fechas mas actuales  
 */

insert into keepcoding_coches.vehiculo  (matricula, kms_total , f_compra , id_marca ,
	id_modelo, id_color, id_seguro, id_revisiones)
SELECT 
 	r.matricula,
    c.kms_totales,
    c.fecha_compra,
    ...
    MAX(r.id)
FROM 
    keepcoding_coches.coches c 
INNER JOIN
    keepcoding_coches.revisiones r  ON r.matricula = c.matricula
INNER JOIN
    keepcoding_coches.color c2 ON c2.color = c.color
INNER JOIN
    keepcoding_coches.modelo m2 ON m2.modelo = c.modelo 
INNER JOIN
    keepcoding_coches.marca m ON  m.marca = c.marca 
inner join 
	keepcoding_coches.seguro s on s.n_poliza = c.n_poliza 
GROUP BY
    r.matricula,
    c.kms_totales,
    c.fecha_compra,
 ...
 /*
  * Ahora tendremos todas las tablas completadas y relacionadas, 
  * a continuación realizo el select que se pide:
  */
   
select m.modelo ,m2.marca ,ge.grupo_empresarial , v.f_compra , v.matricula, 
	c.color , v.kms_total , s.n_poliza , a.aseguradora 
from keepcoding_coches.vehiculo v 
inner join keepcoding_coches.modelo m on m.id = v.id_modelo 
inner join keepcoding_coches.marca m2 on m2.id  = v.id_marca  
inner join keepcoding_coches.grupos_empresariales ge on ge.marca  =  m2.marca 
inner join keepcoding_coches.color c on c.id = v.id_color 
inner join keepcoding_coches.seguro s  on s.id  = v.id_seguro  
inner join keepcoding_coches.aseguradora a  on a.id  = s.id_aseguradora  

/*
 * Este select lo hemos hecho con las uniones de todas las tablas con la de vehiculo, 
 * por lo que nos sale un total de 100 vehiculos, y los datos por ejemplo, de su ultima poliza
 * de seguro y de su correspondiente empresa, ya que algunos habian cambiado de poliza. 
 */
